...
def entry(request, title):
    ...
    if page is not None:
        ...
        # Page doesn't exist, render a template with a message and option to create the page
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if title in entries:
                ...
                # Combining title and content in Markdown format
                content_markdown = f"#{title}\n\n{textarea}"
                # Convert Markdown to HTML
                content_html = markdowner.convert(content_markdown)
                # Save the new entry
...
def edit(request, title):
    if request.method == 'GET':
        ...
        context = {
            'form': Search(),
            'edit': Edit(initial={'title': title, 'textarea': page}),  # Pasar el título inicialmente al formulario
            'title': title
        }
